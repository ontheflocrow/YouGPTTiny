...
def prepare_full(
    ...
    if not filenames:
        raise RuntimeError(
            f"No files matching {wudao_sets[split]} found at {source_path}. \n"
            "Make sure you download the data..."
        )

    builder = packed_dataset.PackedDatasetBuilder(
        outdir=destination_path,
        prefix=f"{split}_wudao_{process_id}",  # Use process_id to differentiate builders
    ...
    for filepath in filenames:
        ...
        with open(filepath, encoding="utf-8") as f:
            ...
            for row in tqdm(texts):
                text_ids = tokenizer.encode(row["content"])
...
def prepare(
    ...
    filenames = glob.glob(os.path.join(source_path, wudao_sets[split]), recursive=True)
