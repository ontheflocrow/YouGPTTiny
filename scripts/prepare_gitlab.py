...
def prepare_full(
    ...
    # if not filenames:
    #     raise RuntimeError(
    #         f"No files matching {gitlab_sets[split]} found at {source_path}. \n"
    #         "Make sure you download the data..."
    #     )

    builder = packed_dataset.PackedDatasetBuilder(
        outdir=destination_path,
        prefix=f"{split}_gitlab_{process_id}",  # Use process_id to differentiate builders
...
def prepare(
    ...
    filenames = glob.glob(os.path.join(source_path, gitlab_sets[split]), recursive=True)
